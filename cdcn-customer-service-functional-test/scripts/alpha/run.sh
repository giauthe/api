...
TESTCASE_FOLDER="../../testcases/get_fee.robot"
...
cd  ./codebase-robot-service/scripts/$ENV
...
echo "=== $(date) - PAYMENT SERVICE- alpha - Running main job === "
python3 -m robot.run -L TRACE -v ENV:$ENV -v browser:headlesschrome -e ignore -e not-ready -d $RESULT_FOLDER $TESTCASE_FOLDER
echo "=== $(date) - PAYMENT SERVICE - alpha - Finished main job !!! Done !!! === "

zip -r ${BUILD_NUMBER}.zip $RESULT_FOLDER
curl -D- -u os_dungnt210:123123a@ -X POST -H "X-Atlassian-Token: nocheck" -F "file=@${BUILD_NUMBER}.zip" -F "comment=This is my File" http://confluence.digital.vn/rest/api/content/46207289/child/attachment
